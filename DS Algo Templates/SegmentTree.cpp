...
public:
	...
	SegTree(
		T *ar, int n, T DEFAULT_VALUE, T (*merge)(T LEFT_NODE, T RIGHT_NODE));
...
template <class T>
T SegTree<T>::queryRec(int L, int R, int SS, int SE, int IDX) {
	if (R < SS || L > SE) {
		return DUMMY_VALUE;
	}
	if (L <= SS && SE <= R) {
...
template <class T>
void SegTree<T>::pointUpdateRec(
	int INDEX, T NEW_VALUE, int SS, int SE, int IDX) {
